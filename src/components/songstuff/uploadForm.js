...
export function UploadForm(){
    ...
    // Fixed: Don't create Audio from File object directly
    useEffect(() => {
        if (mp3File) {
            const audioURL = URL.createObjectURL(mp3File);
            setMp3Audio(new Audio(audioURL));
...
function FormDisplay() {
    ...
    const handleMetadata = async () => {
        ...
        try {
            ...
            const base64File = await fileToBase64(mp3File);
            
            const requestBody = {
                metadata: {
                    MusicId: Date.now(), // Generate a unique ID
...
function InputMp3(){
    ...
    const changeMp3File = (event) => {
        ...
        if(file && file.type === 'audio/mpeg'){
            // Don't create object URL here, do it in the useEffect
            setWarning(`File selected: ${file.name}`);
            setMp3File(file);
        } else if (file) {
            setWarning("Please select an MP3 file");
            setMp3File(null);
        }
    }
